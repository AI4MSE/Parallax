...
<!-- 首屏配图占位 -->
<!-- 内容：左半张图是一只眼睛看到的平面世界（扁平、模糊），右半张图是两只眼睛看到的立体世界（有深度、清晰）。中间大字：Parallax = 深度来自差异 -->
<!-- 风格：简洁科技感，浅色背景，对比强烈 -->
<!-- ![Parallax Concept](assets/parallax_concept.png) -->

---

> AI Agent 自己写完自己检查，就像学生批改自己的作业——永远觉得都对。

**Parallax 是一个很简单的道理：先拆开，再找外人看。每一块都找外人看。**
...
反映在实际使用中：截至 2025 年初的评测，当前最强的模型跑超过 4 小时的任务，成功率不到 10% [3]。微软 2026 年的一项研究也发现，多个主流模型在自主执行文件处理等委托任务时，表现会出现退化 [4]。
...
<!-- 配图占位 -->
<!-- 内容：简洁时间线。关键节点：日心说提出 → 缺乏视差证据争论不休 → 第谷"自洽但错了"（气泡意象）→ 1838 Bessel"换个位置看，终于破案" -->
<!-- 风格：时间线，3-4个关键节点，简洁有力 -->
<!-- ![Heliocentric Timeline](assets/heliocentric_timeline.png) -->
...
<!-- 配图占位 -->
<!-- 内容：Model + Tools + Harness + Parallax 四层示意，Parallax用虚线或渐变表示"正在浮现的新维度" -->
<!-- 风格：简洁架构图 -->
...
<!-- 配图占位 -->
<!-- 内容：两个概念卡片。左卡"他山之石·独立视角"，右卡"化整为零·结构拆解"。中间连线写"Parallax" -->
<!-- 风格：卡片式信息图 -->
<!-- ![Core Insights](assets/core_insights.png) -->
...
<!-- 配图占位 -->
<!-- 内容：左边是一团乱麻（标注"整体审查→看不清"），右边是整齐分格的模块（每格有一只小眼睛图标，标注"逐块审查→无处藏"）-->
<!-- 风格：极简对比图 -->
<!-- ![Break Down](assets/break_down.png) -->
...
<!-- 配图占位 -->
<!-- 内容：5个翻车场景的卡片，每张卡片有问题图标+Parallax解法 -->
<!-- 风格：清晰的信息图 -->
...
<!-- 配图占位 -->
<!-- 内容：完整Parallax Loop流程图。四个阶段（分析→计划→执行→交付），每个阶段右侧有独立审查Agent。左侧标注"人类介入点"。底部标注各阶段对应的Check类型 -->
<!-- 风格：流程图，执行（蓝）和验证（橙）颜色区分 -->
<!-- ![Parallax Loop](assets/parallax_loop.png) -->
...
**[B-Check](skills/bcheck/chs/)** —— 通用独立审查。不管是计划、方案、分析、代码还是文档——任何你觉得需要第二双眼睛看一看的东西，都可以丢给它。它的态度很凶，专挑你不想面对的问题。在完整的 Parallax Loop 里，分析阶段用 B-Check，计划阶段有专门的 P-Check（更侧重方案可行性和拆解合理性）；单独用的话，B-Check 就是一个通用的"找外人看一眼"工具。

**[R-Check](skills/rcheck/chs/)** —— 事后结果验证。做完之后，让一个独立 Agent 拿你的原始需求逐条对照产出。它不管你过程多辛苦，只看结果对不对。
...
<!-- 配图占位 -->
<!-- 内容：四个元素并列。Model（大脑图标）→ Tools（扳手图标）→ Harness（齿轮图标）→ Parallax（双眼/视差图标）。每个下方一句话说明 -->
<!-- 风格：简洁并列，四色区分，Parallax用醒目色 -->
<!-- ![Four Dimensions](assets/four_dimensions.png) -->
